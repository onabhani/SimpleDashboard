...
- **Test**: none — no automated test suite
...
- **`assets/dashboard/node_modules/`** is gitignored — never commit it
- **`wp-config.php`** and `wp-content/uploads/` are gitignored and must not be touched
...
- Commit all changes to the designated feature branch; never push to `main` without permission
